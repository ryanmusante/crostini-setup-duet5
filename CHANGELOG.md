...
2026-08-30  v8.1.43
- Idempotence markers restored on 12 of the 25 managed files. Every write site is gated on `grep -Fq "ry-crostini:${SCRIPT_VERSION}"`, but the emitted content of 12 files never contained that token: nine omitted it outright, and `retroarch.cfg`, `dosbox-x.conf`, `run-x86` and `run-game` wrote `ry-crostini.sh <version>` with a space where the guard expects a colon. Those guards could never match, so the files were rewritten on every run instead of only on a version bump — silently discarding user edits each time, and contradicting both the README "self-heal when SCRIPT_VERSION advances" contract and the in-file "Re-written on version bump" headers. Affected: PipeWire gaming, PipeWire-Pulse gaming, WirePlumber ALSA, `sommelier.conf`, `qt.conf`, `retroarch.cfg`, `scummvm.ini`, `dosbox-x.conf`, `.box64rc`, `run-x86`, `gog-extract`, `run-game`. Each now carries `ry-crostini:@@VERSION@@` in its header comment, matching the 13 files that were already correct. Verified by rendering every heredoc through the real `sed` substitution and running the actual guard against the result: 25/25 match, 0 rewrite-every-run files remain.
- Comment displacement introduced by the v8.1.39/8.1.41/8.1.42 trim passes: 50 script-flow comments described the block before or after the code they sat on. Six sub-step labels were overwritten in the process and are restored — 1g (network reachability), 1i (sommelier), 1p (input group), 2d (/tmp tmpfs cap), 2e (deb822 migration), 9e (timer cleanup); step 1 now reads 1a-1p, step 2 2a-2f, step 7 7a-7h, step 9 9a-9e with no gaps. README already referenced "Step 1p" by name under Known Limitations, so the label was load-bearing documentation, not decoration.
- Seven displaced comments were being emitted INTO generated config files and described the wrong subject: an unterminated GTK/Qt note trailing `sommelier.conf`, an Xft DPI note heading `qt.conf`, an "Input: evdev" note above `video_refresh_rate` in `retroarch.cfg`, RetroArch core-override and BIOS-directory notes inside `scummvm.ini`, an audio-backend note above `cycles=` in the `[cpu]` section of `dosbox-x.conf`, and an ionice note above `export MESA_NO_ERROR=1` in `run-game`. All replaced with accurate text; the `qt.conf` and `scummvm.ini` replacements double as the missing marker headers.
- Five `# shellcheck disable=SC2031` comments cited hardcoded source line numbers (`line 571` twice, `line 560` three times) and all five were wrong — the LOG_FILE-tainting subshell opens at L566. A sixth comment cited `(L827)` for the IS_BOOKWORM assignment, which lives at L834. All six now name `_parallel_check_tools` and the global-init block by symbol, which does not drift when lines move.
- Four comments were factually wrong about the values they annotated: the `/tmp` tmpfs note claimed a 1G cap where the drop-in writes `size=512M` (README says 512 MB); the cros-pin note claimed the unit "pins cros package versions" where it only removes a stale `cros.list`; `BOX64_MAXCPU=0` was labelled "Bigger JIT cache — 512 MB; default 128 MB", but upstream box64 `docs/USAGE.md` defines `BOX64_MAXCPU` as maximum CPU cores exposed with 0 as the default — the 512/128 figures belong to `BOX64_DYNAREC_FORWARD`, which was itself carrying a `BOX64_DYNAREC_NATIVEFLAGS` note for a variable this file never sets. All four rewritten against the shipped values and upstream documentation.
- `check_tool` referenced `_TOOL_OVERRIDES` in a comment; no such identifier exists anywhere in the script. Replaced with an accurate description of the empty-override (present-only) path it actually guards.
- Write tmpfiles orphaned by a signal in the `mktemp`..`mv` window. `_write_file_impl` tracked nothing at all, and the `_SUDO_TMPFILE` variable that `write_file_sudo` does track cannot work at 5 of its 7 call sites: 24 of the 26 write call sites are pipeline-fed (`printf ... | write_file path`), and a pipeline subshell cannot assign to the parent shell, so the trap always saw an empty variable. Replaced with PID-scoped tmpfile names (`.tmp_${$}_XXXXXX`, and `$$` does survive a subshell) plus a sweep in `cleanup` over `$HOME` and the four managed `/etc` roots, which works regardless of subshell scope. The `_SUDO_TMPFILE` block is kept for the two redirect-fed sites that do run in the main shell. Measured over 8 mid-write SIGINTs: 4 orphans on 8.1.42, 0 on 8.1.43; over 20 iterations, 0. Confirmed the sweep is PID-scoped and bounded — a foreign-PID tmpfile, a legacy-named tmpfile, an unrelated sibling file, and a matching name outside `$HOME` all survive untouched, and the `/etc` pass uses `sudo -n` so it can never prompt from inside a trap.
- Cleanup trap told the user to "Re-run to resume from checkpoint" for failures that happened before the lock was taken — argument errors, the `--reset` non-interactive refusal — where no step had run and no checkpoint existed. Gated on `$_LOCK_ACQUIRED`; pre-lock failures now report that nothing was changed.
- Housekeeping: 4 comments carried a stray leading space that broke indentation alignment; a stale "libavcodec-extra is now included above" note was removed; the two-line fontconfig comment block was collapsed to one line per the v8.1.39 convention; `optimisation` -> `optimization` in `.box64rc`, `catalogue` -> `catalog` and `defence` -> `defense` in the docs.
- README — `run-x86 --help` was documented as listing available backends; it prints a fixed usage string and the dispatch order, and the text now says so. Signal-handling row updated to reflect that both plain and sudo write tmpfiles are tracked.
- Validation: `bash -n` clean; `shellcheck -S style` clean except the 4 documented SC2030 subshell-scope notes. Every guarded user-writable file was exercised end-to-end through its real guard-and-write block: 19/19 write on run 1 and skip on run 2, 19/19 emitted artifacts carry the marker, modes 644 config / 700 wrapper, 0 stray tmpfiles. Generated wrappers pass `bash -n`, report `v8.1.43`, and honor exit 2 on usage errors; the generated `fonts.conf` parses as XML. Signals exit 130/143/129/131 with the lock released. 25/25 marker guards satisfied; 0 hardcoded line references; 0 duplicate script-flow comment texts; 0 dangling comment referents; 0 stacked comment blocks; 0 comment-indent anomalies; sub-step label sequences contiguous; 0 CR bytes, 0 trailing whitespace, 0 tabs, 0 script-flow comments over 120 bytes. `--version` reports 8.1.43. Line count 3191 -> 3198.

2026-04-16  v8.1.42
- Comment trimming. 102 comment lines exceeding 120 bytes tightened to ≤120 bytes each. Technical content (byte offsets, URLs, kernel/box64 version pins, CLI flags, CPU part IDs) preserved; verbose explanatory prose removed. Heredoc content (config files, embedded wrapper scripts) and `# shellcheck` directives untouched per the v8.1.39 convention.
- CHANGELOG — audit-meta language removed: 9 preamble bullets (`Audit fixes. Exhaustive pass on vX surfaced N HIGH/MED/LOW...`, `After re-verification, N findings were retracted`, `- Retracted — ...`) dropped across all historical entries. 36 fix bullets had their severity prefix (`- HIGH —`, `- MED —`, `- LOW —`, `- INFO —`, `- POLISH —`, `- SC####—`) stripped, leaving plain bullets describing what changed.
- Validation: `bash -n` clean; `shellcheck -S warning` clean; 2 documented SC2030 notes remain. `--version` reports 8.1.42. No code-path changes; comments and CHANGELOG only.

2026-04-16  v8.1.41
- Comment trimming. Header block at L2-12 (11 lines of prose separated by blank `#` paragraph breaks) collapsed to a single line. Secondary 2-line block at L224-225 (`# Logging` + `_LOG_READY` rationale) merged into one line with em-dash separator. No logic changes; heredoc content and `# shellcheck` directives untouched.
- Validation: `bash -n` clean; `shellcheck -S warning` clean; 2 documented SC2030 notes remain. `--version` reports 8.1.41.

2026-04-16  v8.1.40
- L234 (v8.1.39) cleanup trap + pre-creation exit → 0644 log file: any `exit` path before the umask-077 subshell created LOG_FILE (e.g. typo `--helpp`, `date` failure at L25, read-only $HOME) triggered the EXIT trap's `_cleanup_warn` → `warn()` → `printf ... >> "$LOG_FILE"`, which created the file at the process's default umask (0022 → 0644). This is a security regression vs. the required 0600 for a file that accumulates sudo command output, env var dumps, and system config. Not hypothetical — repro'd end-to-end: `bash ry-crostini.sh --helpp` left `~/ry-crostini-*.log` at 0644 containing the WARN line. Fix: added `_LOG_READY=false` sentinel at global init, flipped `true` only after the umask-077 subshell creates LOG_FILE successfully; gated all `>> "$LOG_FILE"` writes in `log`/`warn`/`err`/`_cleanup_warn` on `$_LOG_READY`. Pre-creation exits now write only to stderr and leave no file on disk. Repro confirms.
- L334 `get_checkpoint` missing upper-bound: validated STEP_FILE content with `^[0-9]+$` but no `<= 13` check. A corrupted or hand-edited checkpoint of `999` passed the regex; `should_run_step N` (N > 999) was always false; all 13 steps silently skipped; `_verify_pass+_verify_fail+_verify_warn=0` triggered the "NO CHECKS RAN" summary and the script exited 0 with nothing done. Fix: tightened validator to `[[ "$val" =~ ^[0-9]+$ ]] && [[ "$((10#$val))" -le 13 ]]`; `10#` prefix defends against an edited file with a leading zero (`08` / `09` would otherwise hit the bash octal trap under arithmetic). Invalid values now emit a WARN, fall back to 0, and run from step 1. Repro confirms 999/14/08 all handled; 13 (valid max) cleanly skips; 012 correctly resolves to decimal 12.
- L710 pre-scan argv loop silently ignored unknown `-*` options: `bash ry-crostini.sh --helpp` (typo) passed through the pre-scan without matching any case arm, reached the LOG_FILE creation block which successfully created the file, then died in the main parser with "Unknown option". Combined with the 0644 regression above, this left every typo as an orphan. Fix: added explicit allowlist (`--interactive|--upgrade-trixie|--from-step=*|--from-step|--verify|--force`) plus catch-all `-*) printf 'ERROR: ...' >&2; exit 1` in the pre-scan. Known options pass through to the main parser unchanged; unknown `-*` dies immediately before LOG_FILE creation. Bare-word args still defer to the main parser so its error message wins. Also tracks `--` sentinel with a `$_pre_past_ddash` flag so `bash ry-crostini.sh -- --helpp` does not falsely reject (everything past `--` is ignored).
- L735 `--from-step=` error messages: prior single message `--from-step requires a number 1-13 (got '${_from}')` rendered as `(got '')` for empty, which reads like a bug in the error handler itself. Split into three distinct messages — `got empty value` / `got non-numeric 'abc'` / `got out-of-range '99'` — so the user immediately knows which malformation to fix.
- L236,L242,L248,L296 `$_LOG_READY && printf ... >> "$LOG_FILE" 2>/dev/null || true` flagged `A && B || C is not if-then-else`. The logic was correct (both branches safe) but the idiom is fragile under refactor. Fix: converted all four sites to `if $_LOG_READY; then ...; fi` form. Shellcheck now clean at `-S warning` and `-S info`; only the 2 documented SC2030 subshell-scope notes remain (intentional parallel counter pattern).
- Comment trimming. Two multi-line comment blocks introduced in v8.1.40 (the `_LOG_READY` rationale at L225 and the pre-scan rationale at L694) collapsed to single lines per the v8.1.39 convention. Detailed rationale preserved in this CHANGELOG entry.
- Validation: `bash -n` clean; `shellcheck -S warning` clean; `shellcheck` default clean except the 2 documented SC2030 notes. End-to-end repro: `--helpp`, `--from-step=`, `--from-step=abc`, `--from-step=99`, `--from-step=13`, corrupt checkpoint 999/14/012/08, legitimate `--help` / `--version` — all behave as designed, none leave an orphan log, `--version` run leaves $HOME empty. Line count: 3184 → 3202 (+18 net for sentinel init, gating, argv allowlist, validator tightening, error-message split; offset by the two comment trims).
...
2026-04-16  v8.1.38
- L1083,1102,1118,1133,1147,1159 `_unused` variable leak: interactive prompts used `read -r -t 300 _unused </dev/tty` six times but never unset the variable, leaking it into scope for the rest of the script. Fix: replaced `_unused` with `_` (bash throwaway variable), eliminating the leak entirely.
- L1227 redundant `IS_BOOKWORM=true` inside step 2: identical assignment already performed at global init L827 under the same condition (`!UPGRADE_TRIXIE && codename==bookworm`). Fix: removed the redundant assignment; added a comment cross-referencing L827.
- L1436 `mv` without `--` separator in systemd unit ExecStart: all other mv calls in the script use `mv --` for defensive path handling. The operands here are fixed literal paths (`/etc/apt/sources.list.d/cros.list` → `/etc/apt/cros.list.regenerated`) so risk was zero, but the inconsistency conflicted with the project's coding standard. Fix: added `--`.
...
- README docs — Cloud Gaming table: Amazon Luna split out to its own ⚠ Prime-only row noting the 2026-04-10 storefront/BYOL/third-party-subscription shutdown and the 2026-06-10 a-la-carte streaming sunset. Luna remains viable for Prime-tier and Luna Premium subscribers on the ~155-title catalog.
...
- Step 9 journald: added `SystemMaxUse=50M` / `SystemMaxFileSize=10M` as defense-in-depth (currently unreachable under `Storage=volatile`).
