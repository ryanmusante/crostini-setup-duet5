#!/usr/bin/env bash
# ry-crostini.sh v8.1.43 — Crostini post-install bootstrap for Lenovo Duet 5 (SC7180P aarch64). See README.md.
...
readonly SCRIPT_VERSION="8.1.43"
...
cleanup() {
    ...
    _strip_log_ansi
    # Stop sudo credential keepalive (disowned; kill by raw PID — no wait possible after disown).
    ...
    # Signal backgrounded _parallel_check_tools children before rm'ing their output dir (avoids write-races).
    if [[ "${#_PARALLEL_PIDS[@]}" -gt 0 ]]; then
        kill "${_PARALLEL_PIDS[@]}" 2>/dev/null || true
        wait "${_PARALLEL_PIDS[@]}" 2>/dev/null || true
        _PARALLEL_PIDS=()
    fi
    ...
    # PID-scoped tmpfile sweep — 24 of 26 write sites are pipeline-fed, and a subshell cannot assign to the parent.
    find "$HOME" -name ".tmp_${$}_*" -type f -delete 2>/dev/null || true
    sudo -n find /etc/apt /etc/systemd /etc/default /etc/profile.d -maxdepth 3 \
        -name ".tmp_${$}_*" -type f -delete 2>/dev/null || true
    # Abandoned write_file_sudo tmpfile from the 2 redirect-fed sites. Name-validated before sudo rm.
    ...
    if [[ "$rc" -ne 0 ]]; then
        if [[ "${_had_failures:-0}" -gt 0 ]]; then
            ...
            if $_LOCK_ACQUIRED; then
                ...
                _cleanup_warn "Script exited with code $rc before any step ran. Nothing was changed."
            fi
        fi
    fi
    # Re-raise caught signal for 128+N exit code. Allowlist defends against $_received_signal clobber.
...
_progress_resize() {
    ...
    # Floor mirrors _progress_init. Disable progress + reset scroll region if terminal <5 rows.
    if [[ "$rows" -lt 5 ]]; then
        _PROGRESS_ENABLED=false
        printf '\033[r'
        return
...
# Logging — _LOG_READY gates LOG_FILE writes until the umask-077 0600 subshell creation; prevents 0644 orphans.
log() {
    local msg
    # printf '%(%T)T' -1 uses bash 4.2+ builtin — no date(1) fork
    printf -v msg '%(%T)T [INFO]  %s' -1 "$*"
    if $_LOG_READY; then printf '%s\n' "$msg" >> "$LOG_FILE" 2>/dev/null || true; fi
    printf '%s\n' "$msg"
}
warn() {
    local msg
    printf -v msg '%(%T)T [WARN]  %s' -1 "$*"
    if $_LOG_READY; then printf '%s\n' "$msg" >> "$LOG_FILE" 2>/dev/null || true; fi
    printf '%s\n' "$msg" >&2
}
err() {
    local msg
    printf -v msg '%(%T)T [ERROR] %s' -1 "$*"
    if $_LOG_READY; then printf '%s\n' "$msg" >> "$LOG_FILE" 2>/dev/null || true; fi
    printf '%s\n' "$msg" >&2
}
die()  { err "$*"; exit 1; }

# _read_os_release: print one /etc/os-release field with fallback. Sourced in a subshell.
_read_os_release() {
...
# _has_capture_dev: true if /dev/snd has any ALSA capture node (pcmC*D*c). Glob-based to avoid pipefail false-negatives.
_has_capture_dev() {
    [[ -d /dev/snd ]] || return 1
    local _dev
...
_strip_log_ansi() {
    ...
    # mktemp on Linux creates 0600. Requires GNU sed for \x hex escapes (Crostini ships GNU sed).
...
get_checkpoint() {
    ...
    if [[ -f "$STEP_FILE" ]]; then
        local val
        # read builtin — no cat fork; redirection failure caught by || even under inherit_errexit.
        { read -r val < "$STEP_FILE"; } 2>/dev/null || { warn "Cannot read checkpoint file ${STEP_FILE}. Use --reset to clear."; echo 0; return 0; }
        if [[ "$val" =~ ^[0-9]+$ ]] && [[ "$((10#$val))" -le 13 ]]; then
            echo "$((10#$val))"
        else
            warn "Invalid checkpoint value '${val}' (must be 0-13). Use --reset to clear."
            echo 0
        fi
    else
        echo 0
...
# _tee_log: drop three known-harmless dpkg/uevent/systemctl noise lines, then append to the log
_tee_log() {
...
_write_file_impl() {
    local dest="$1" mode="$2"
    # Refuse to clobber a destination symlink. Test on $dest must precede mktemp to prevent tmpfile leaks.
    [[ -L "$dest" ]] && die "Refusing to write $dest: destination is a symlink"
    mkdir -p "$(dirname "$dest")" || die "Cannot create parent dir for $dest"
    local tmp
    tmp="$(mktemp "$(dirname "$dest")/.tmp_${$}_XXXXXX")" || die "Cannot create tmpfile for $dest"
...
write_file_sudo() {
    local dest="$1"
    # Refuse to clobber a destination symlink (parity with _write_file_impl). Test on $dest must precede mktemp.
    ...
    local tmp
    tmp="$(sudo mktemp "$(dirname "$dest")/.tmp_${$}_XXXXXX")" || die "Cannot create tmpfile for $dest"
...
check_tool() {
    ...
    if command -v "$cmd" &>/dev/null; then
        ...
        # An empty override means present-only: skip the version probe entirely (see the _bad branch).
        if [[ -n "$flag" ]]; then
...
_parallel_check_tools() {
    ...
    # Parallel tool check via subshell + SOH-sentinel counter aggregation. Subshell-scope taint (SC2030) is intentional.
    _PARALLEL_TMPDIR="$_pct_dir"

    local _pct_entry _pct_idx
    for _pct_entry in "$@"; do
        printf -v _pct_idx '%04d' "$_pct_n"
        # Subshell reassigns LOG_FILE + _verify_* counters; parent re-aggregates via SOH sentinel. SC2030/2031 expected.
        (
            # Silence direct log writes in the child — the parent replays and logs this output after wait
            LOG_FILE=/dev/null
    ...
    # Expose pids to cleanup trap so SIGINT during wait kills children before rm -rf races the tmpdir
    _PARALLEL_PIDS=("${_pct_pids[@]}")
    ...
    for _pct_f in "${_pct_dir}/"*; do
        ...
        while IFS= read -r _pct_line; do
            if [[ "$_pct_line" == $'\x01'* ]]; then
                ...
                # shellcheck disable=SC2031  # main-shell scope; the _parallel_check_tools taint is contained
...
EXIT CODES:
    0    Success (all steps completed; verification passed if run)
    1    Failure (verification failures or fatal error during a step)
    2    No verification checks executed (e.g. --from-step=13 on a fresh run)
...
# Pre-scan argv for --help/--version/--reset before LOG_FILE creation; rejects unknown -* typos.
_pre_reset=false
_pre_past_ddash=false
for _arg in "$@"; do
    if $_pre_past_ddash; then
        continue
    fi
    case "$_arg" in
        --)        _pre_past_ddash=true ;;
        --help)    usage ;;
        --version) echo "${SCRIPT_NAME} v${SCRIPT_VERSION}"; exit 0 ;;
        --reset)   _pre_reset=true ;;
        # Known long options — listed so unknown -* falls through to catch-all. Main parser handles them.
        --interactive|--upgrade-trixie|--from-step=*|--from-step|--verify|--force) ;;
        -*)        printf 'ERROR: unknown option: %s. Use --help for usage.\n' "$_arg" >&2; exit 1 ;;
        *)         ;;  # bare-word args: let main parser report
    esac
done
unset _arg _pre_past_ddash

# Create log file at mode 600 atomically via umask subshell. Deferred past --help/--version. --reset skips creation.
# shellcheck disable=SC2031  # LOG_FILE is main-shell here; the taint is from _parallel_check_tools subshells
if ! $_pre_reset && ! ( umask 077; : > "$LOG_FILE" ) 2>/dev/null; then
    printf 'FATAL: cannot create log file %s\n' "$LOG_FILE" >&2
    exit 1
fi
# Log file now exists at 0600 (or will never be created, if --reset). Allow log writes.
$_pre_reset || _LOG_READY=true
unset _pre_reset
...
for arg in "$@"; do
    case "$arg" in
        ...
        --from-step=*)
            ...
            _from="${arg#*=}"
            if [[ -z "$_from" ]]; then
                die "--from-step=N requires a positive integer 1-13 (got empty value)"
            elif [[ ! "$_from" =~ ^[0-9]+$ ]]; then
                die "--from-step=N requires a positive integer 1-13 (got non-numeric '${_from}')"
            elif [[ "$((10#$_from))" -lt 1 ]] || [[ "$((10#$_from))" -gt 13 ]]; then
                die "--from-step=N requires a positive integer 1-13 (got out-of-range '${_from}')"
            fi
        ...
        --reset)
            ...
            unset _a
            # Probe lock for liveness; DEFER removal until confirmation — "n" answer must not destroy the lock.
            if [[ -d "$LOCK_FILE" ]]; then
            ...
            ;;
        --force)
            # Only meaningful with --reset; warn on standalone so `--forced` typo does not silently do nothing.
            _has_reset=false
...
# Mark ownership immediately after mkdir — closes the signal window that would orphan the lock dir.
_LOCK_ACQUIRED=true
...
# Codename detected once at global init; steps 2/3/6/8/10 branch on IS_BOOKWORM
...
# sudo credential keepalive — refresh every 60 s; abort the run after 15 consecutive failures.
(
    _ka_fails=0
    while true; do
...
_SUDO_KEEPALIVE_PID=$!
# Detach the keepalive so it survives without a wait and never blocks the EXIT trap
disown

# Initialize progress bar (requires terminal, checkpoint, and color globals)
_progress_init

# Rotate old log files — keep last 7 days. Also sweeps orphaned .log.strip_* tmpfiles from prior killed runs.
...
_gpu_conf_content() {
    cat <<'EOF'
# Crostini GPU acceleration environment — managed by ry-crostini.sh ry-crostini:@@VERSION@@ Wayland EGL
EGL_PLATFORM=wayland
# GTK4 dark mode
GTK_THEME=Adwaita:dark
# Mesa env — MESA_GLTHREAD and MESA_NO_ERROR intentionally not set globally; enabled per-game via run-game wrapper.
GSK_RENDERER=ngl

# Force virtio_gpu DRI driver — Mesa on trixie 24.x may pick Zink and crash. Filename is virtio_gpu_dri.so.
MESA_LOADER_DRIVER_OVERRIDE=virtio_gpu

# MESA_NO_ERROR omitted — invalid GL calls on virgl cross VM boundary and can hang host virglrenderer.

# Shader cache DB respects MAX_SIZE (Fossilize single-file ignores). 256 MB cap fits 128 GB eMMC.
...
# mesa_glthread omitted — virgl serializes GL through virtio-gpu; glthread just adds overhead.
EOF
}
...
_pw_gaming_content() {
    cat <<'PWEOF'
# PipeWire core for Crostini gaming — ry-crostini:@@VERSION@@. Counteracts KVM quantum=1024; 512@48k = 10.67 ms.
...
_pw_pulse_gaming_content() {
    cat <<'PPEOF'
# PipeWire PulseAudio layer — ry-crostini:@@VERSION@@. pulse.properties.rules replaces vm.overrides (PW 1.4.x).
...
_gtk_settings_content() {
    cat <<'GTKEOF'
# GTK 3/4 settings — managed by ry-crostini.sh — ry-crostini:@@VERSION@@
[Settings]
...
# Release cros-* holds left behind by an interrupted prior run, before any apt work starts
...
if should_run_step 1; then
    ...
    # 1g. Network reachability (deb.debian.org)
    if ! command -v curl &>/dev/null; then
        warn "curl not present — skipping network reachability probe (installed in step 3)"
    ...
    log "Running as user: ${USER:-$(id -un)} ✓"

    # 1i. Sommelier (Wayland bridge)
    ...
    # 1p. Gamepad access — add the user to the 'input' group
    _ry_user="${USER:-$(id -un)}"
    if [[ " $(id -nG "$_ry_user") " != *" input "* ]]; then
        if run sudo usermod -aG input "$_ry_user"; then
            log "Added $_ry_user to 'input' group ✓ — log out and back in for gamepad access"
        else
            warn "Could not add $_ry_user to 'input' group — gamepads may not work in RetroArch/DOSBox-X"
        fi
    else
        log "User $_ry_user already in 'input' group ✓"
    fi
    unset _ry_user
...
EOF
    ...
    # Empty check: --from-step=2 skips step 1's VERSION_CODENAME validation. Re-validate here.
    ...
    if ! $UPGRADE_TRIXIE; then
        ...
        if [[ "$_cur_codename" == "bookworm" ]]; then
            # IS_BOOKWORM already set at global init; no redundant assignment needed
            log "Enabling bookworm-backports for pipewire 1.4 / wireplumber 0.5"
            _BPO_LIST="/etc/apt/sources.list.d/bookworm-backports.list"
            # bookworm-backports — enables pipewire 1.4 / wireplumber 0.5 on bookworm without a full trixie upgrade.
            if [[ ! -f "$_BPO_LIST" ]] && ! grep -rEq '^[[:space:]]*(deb|URIs:|Suites:)[^#]*bookworm-backports' /etc/apt/sources.list /etc/apt/sources.list.d/ 2>/dev/null; then
    ...
    elif [[ "$_cur_codename" == "bookworm" ]]; then
        log "Current release: ${_cur_codename} — upgrading to Trixie (Debian 13)"
        _did_trixie_rewrite=true
        # bookworm-backports would pin a trixie host to bookworm packages — move it aside first
        ...
        # Rewrite the legacy sources.list; back it up first (first-backup-wins across retries)
        ...
        for _sfile in /etc/apt/sources.list.d/*.list /etc/apt/sources.list.d/*.sources; do
            [[ -f "$_sfile" ]] || continue
            # Skip *-backports source — mechanical rewrite to trixie-backports may not exist at upgrade time.
    ...
    # Release the cros-* holds taken above; cros-guest-tools stays held on trixie
    _CROS_UNHOLD_PKGS=()
    ...
    # 2d. /tmp tmpfs cap — trixie mounts /tmp as tmpfs; cap at 512M to protect 4 GB of RAM
    if $IS_BOOKWORM; then
        log "Skipping /tmp tmpfs cap on bookworm (disk-backed /tmp)"
    else
        _TMP_DROPIN="/etc/systemd/system/tmp.mount.d/override.conf"
        if [[ ! -f "$_TMP_DROPIN" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "$_TMP_DROPIN" 2>/dev/null; then
            sed "s/@@VERSION@@/${SCRIPT_VERSION}/" <<'TMPEOF' | write_file_sudo "$_TMP_DROPIN"
# ry-crostini:@@VERSION@@
[Mount]
...
TMPEOF
            ...
            log "tmp.mount drop-in up-to-date"
        fi
        unset _TMP_DROPIN
    fi

    # 2e. deb822 migration — apt >= 2.9 modernize-sources rewrites .list into .sources
    ...
    if [[ ! -f "$_CROS_PIN_SVC" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "$_CROS_PIN_SVC" 2>/dev/null; then
        sed "s/@@VERSION@@/${SCRIPT_VERSION}/" <<'CROSEOF' | write_file_sudo "$_CROS_PIN_SVC"
# ry-crostini:@@VERSION@@
[Unit]
...
CROSEOF
        ...
        log "ry-crostini-cros-pin.service up-to-date"
    fi
    unset _CROS_PIN_SVC
...
if should_run_step 3; then
    ...
    CORE_PKGS=(
        ...
        # Network and TLS
        curl wget dnsutils openssh-client iputils-ping
    ...
    # `host` command: bookworm ships bind9-host; trixie splits it into bind9-dnsutils.
    if $IS_BOOKWORM; then
        install_pkgs_best_effort bind9-host || warn "bind9-host install failed — host(1) unavailable"
    else
        install_pkgs_best_effort bind9-dnsutils || warn "bind9-dnsutils install failed — host(1) unavailable"
    fi

    # 7z source differs by codename: bookworm ships p7zip-full, trixie ships 7zip 24.x
    if $IS_BOOKWORM; then
    ...
    if run sudo DEBIAN_FRONTEND=noninteractive apt-get install -y earlyoom; then
        ...
        if [[ ! -f "$_EARLYOOM_CONF" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "$_EARLYOOM_CONF"; then
            # Direct interpolation — sed corrupted the value because _EOOM_PREFER contains the sed delimiter '|'.
            printf '%s\n' \
                "# earlyoom config — managed by ry-crostini.sh" \
                "# ry-crostini:${SCRIPT_VERSION}" \
                "EARLYOOM_ARGS=\"-m 10 -s 100 -r 3600 --prefer (${_EOOM_PREFER}) --avoid (^|/)(init|systemd|dbus-daemon|garcon|sommelier)\$ --sort-by-rss\"" \
                | write_file_sudo "$_EARLYOOM_CONF"
            # Fail loudly if the --prefer regex did not survive the write (delimiter corruption)
...
if should_run_step 5; then
    ...
    # Pre-create Mesa shader cache dir — removes a concurrent-launch race on first run.
...
if should_run_step 6; then
    ...
    # bookworm only: refresh PipeWire/WirePlumber from backports so WP 0.5 JSON is honored
    if $IS_BOOKWORM; then
    ...
    # PipeWire supersedes PulseAudio — mask the daemon if the old package is still installed
    if [[ "$(dpkg-query -W -f='${Status}' pulseaudio 2>/dev/null)" == "install ok installed" ]]; then
    ...
    # Track whether any audio config changed, to decide on a PipeWire restart below
...
# WirePlumber ALSA tuning for Crostini gaming — ry-crostini:@@VERSION@@. WP 0.5+ JSON. Disables auto-suspend.
...
WPEOF
    ...
    # Restart the audio stack only when a config actually changed
...
if should_run_step 7; then
    ...
    if [[ ! -f "$SOMMELIER_ENV" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "$SOMMELIER_ENV" 2>/dev/null; then
        sed "s/@@VERSION@@/${SCRIPT_VERSION}/" <<'SOMMEOF' | write_file "$SOMMELIER_ENV"
# Sommelier display scaling — ry-crostini:@@VERSION@@. SOMMELIER_SCALE=1.0 native; 0.5 = 2x magnify for 4K.
SOMMELIER_SCALE=1.0

# Super key passthrough — required for ChromeOS launcher key to reach Linux apps (wayland only).
SOMMELIER_ACCELERATORS=Super_L
SOMMEOF
    else
        log "Sommelier env up-to-date — skipping"
    fi

    # 7b. GTK 3 settings
    GTK3_SETTINGS="${HOME}/.config/gtk-3.0/settings.ini"
    if [[ ! -f "$GTK3_SETTINGS" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "$GTK3_SETTINGS" 2>/dev/null; then
        _gtk_settings_content | sed "s/@@VERSION@@/${SCRIPT_VERSION}/" | write_file "$GTK3_SETTINGS"
    else
        log "GTK 3 settings.ini up-to-date — skipping"
    fi

    # 7c. GTK 4 settings
    GTK4_SETTINGS="${HOME}/.config/gtk-4.0/settings.ini"
    if [[ ! -f "$GTK4_SETTINGS" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "$GTK4_SETTINGS" 2>/dev/null; then
        _gtk_settings_content | sed "s/@@VERSION@@/${SCRIPT_VERSION}/" | write_file "$GTK4_SETTINGS"
    else
        log "GTK 4 settings.ini up-to-date — skipping"
    fi

    # 7d. GTK 2 settings (legacy apps)
    GTK2_RC="${HOME}/.gtkrc-2.0"
    if [[ ! -f "$GTK2_RC" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "$GTK2_RC" 2>/dev/null; then
        sed "s/@@VERSION@@/${SCRIPT_VERSION}/" <<'GTK2EOF' | write_file "$GTK2_RC"
# GTK 2 theme settings for legacy apps — managed by ry-crostini.sh — ry-crostini:@@VERSION@@
gtk-theme-name="Adwaita-dark"
gtk-icon-theme-name="Adwaita"
gtk-font-name="Noto Sans 11"
gtk-cursor-theme-name="Adwaita"
...
GTK2EOF
        ...
        log ".gtkrc-2.0 up-to-date — skipping"
    fi

    # 7e. Qt scaling and theming
    QT_ENV="${HOME}/.config/environment.d/qt.conf"
    if [[ ! -f "$QT_ENV" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "$QT_ENV" 2>/dev/null; then
        sed "s/@@VERSION@@/${SCRIPT_VERSION}/" <<'QTEOF' | write_file "$QT_ENV"
# Qt platform theme and Wayland scaling — managed by ry-crostini.sh — ry-crostini:@@VERSION@@
QT_QPA_PLATFORM=wayland;xcb
QT_WAYLAND_DISABLE_WINDOWDECORATION=1
QT_QPA_PLATFORMTHEME=gtk3
QTEOF
    else
        log "Qt env up-to-date — skipping"
    fi
    ...
    if [[ ! -f "$XRESOURCES" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "$XRESOURCES" 2>/dev/null; then
        sed "s/@@VERSION@@/${SCRIPT_VERSION}/" <<'XRESEOF' | write_file "$XRESOURCES"
! Font rendering for X11 apps on Duet 5 (13.3in 1920x1080 OLED) — managed by ry-crostini.sh
! ry-crostini:@@VERSION@@
...
XRESEOF
        ...
        log ".Xresources up-to-date — skipping"
    fi
    ...
    # 7g. Fontconfig — grayscale AA for OLED; Noto defaults; monospace alias to Fira Code.
...
FCEOF
    ...
    if [[ ! -f "${CURSOR_DIR}/index.theme" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "${CURSOR_DIR}/index.theme" 2>/dev/null; then
        sed "s/@@VERSION@@/${SCRIPT_VERSION}/" <<'CURSEOF' | write_file "${CURSOR_DIR}/index.theme"
# ry-crostini:@@VERSION@@
[Icon Theme]
Inherits=Adwaita
CURSEOF
    else
        log "Cursor theme up-to-date — skipping"
    fi
...
if should_run_step 8; then
    ...
    # shellcheck disable=SC2031  # LOG_FILE is main-shell here; _parallel_check_tools taint is contained
...
# Step 9: Container resource tuning
...
ForwardToSyslog=no
# Drop info/debug messages — saves RAM on 4 GB system; warning+ still kept
MaxLevelStore=warning
# No compression — small volatile journal, save CPU on Cortex-A55 efficiency cores
Compress=no
# Audit subsystem unreachable in unprivileged Crostini LXC; skip the kernel call
Audit=no
JDEOF
...
# Parallel make — cap at 4 big cores (run-game affinity). Protects 4 GB RAM. -l prevents thrashing.
_ry_nproc="$(nproc 2>/dev/null || echo 2)"
[ "${_ry_nproc:-2}" -gt 4 ] && _ry_nproc=4
export MAKEFLAGS="-j${_ry_nproc:-2} -l${_ry_nproc:-2}.0"
unset _ry_nproc
...
ENVEOF
    ...
    # 9e. Timer cleanup — mask the unattended-upgrade and maintenance timers
...
if should_run_step 10; then
    ...
    # Emulators and unpackers; dosbox-x and box64 are trixie-only
    if $IS_BOOKWORM; then
    ...
    # unrar is non-free and often absent — probe the candidate before attempting an install
    _unrar_cand="$(LC_ALL=C apt-cache policy unrar 2>/dev/null | awk '/Candidate:/ {print $2}')"
    ...
    # RetroArch default config (self-heals on version bump)
    _RA_CFG="${HOME}/.config/retroarch/retroarch.cfg"
    if [[ ! -f "$_RA_CFG" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "$_RA_CFG" 2>/dev/null; then
        sed "s/@@VERSION@@/${SCRIPT_VERSION}/" <<'RACFG' | write_file "$_RA_CFG"
# RetroArch Crostini config — managed by ry-crostini.sh — ry-crostini:@@VERSION@@. Re-written on version bump.

# Video: glcore on virgl's GL 4.3 core; enables slang shaders. video_threaded off (breaks frame_delay_auto).
video_driver = "glcore"
...
# Audio: ALSA via PipeWire compat. Native PW driver has broken latency in RA 1.20.0 (libretro/RetroArch#17685).
audio_driver = "alsa"
audio_latency = "32"

# Video: refresh-rate hint for frame pacing; matches the Crostini compositor
...
# Memory: disable rewind (~20 MB/min on 4 GB). Preemptive Frames: 8/16-bit only. Uses run_ahead_frames.
rewind_enable = "false"
...
# Prevent accidental overwrite of these tuned defaults when exiting via menu (upstream default is true)
config_save_on_exit = "false"

# Cap fast-forward at 3× — uncapped (default 0.0) thermally throttles SC7180P and wastes battery
fastforward_ratio = "3.0"

# Pixel-perfect output for retro content on OLED (upstream defaults: smooth=true, integer=false)
video_smooth = "false"
video_scale_integer = "true"
RACFG
    else
        log "RetroArch config up-to-date — skipping"
    fi
    unset _RA_CFG

    # ScummVM default config (self-heals on version bump)
    _SVM_CFG="${HOME}/.config/scummvm/scummvm.ini"
    if [[ ! -f "$_SVM_CFG" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "$_SVM_CFG" 2>/dev/null; then
        sed "s/@@VERSION@@/${SCRIPT_VERSION}/" <<'SVMCFG' | write_file "$_SVM_CFG"
# ScummVM Crostini config — managed by ry-crostini.sh — ry-crostini:@@VERSION@@. Re-written on version bump.
[scummvm]
gfx_mode=opengl
stretch_mode=pixel-perfect
# Aspect-ratio correction for 320x200 titles on a 16:9 panel
aspect_ratio=true
filtering=false
vsync=true
...
SVMCFG
        ...
        log "ScummVM config up-to-date — skipping"
    fi
    unset _SVM_CFG

    # DOSBox-X default config (self-heals on version bump)
    ...
    elif [[ ! -f "$_DBX_CFG" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "$_DBX_CFG" 2>/dev/null; then
        sed "s/@@VERSION@@/${SCRIPT_VERSION}/" <<'DBXCFG' | write_file "$_DBX_CFG"
# DOSBox-X Crostini config — managed by ry-crostini.sh — ry-crostini:@@VERSION@@. Re-written on version bump.
...
[cpu]
# ARM64 dynarec — 3-4x speedup. Verify dosbox-x --version shows C_DYNREC 1. Falls back to normal for 386 PM paging.
core=dynamic_rec
# Cycle target with a 70% limit — leaves headroom on the A55 efficiency cores
...
DBXCFG
        ...
        log "DOSBox-X config up-to-date — skipping"
    fi
    unset _DBX_CFG
    ...
    # box64 is trixie-only (not in bookworm repos); run-x86 falls back to qemu-user there
    if $IS_BOOKWORM; then
        ...
        _box64_cand="$(LC_ALL=C apt-cache policy box64 2>/dev/null | awk '/Candidate:/ {print $2}')"
    ...
    # Write ~/.box64rc with SC7180P defaults (skip on bookworm — box64 not installed). Self-heals on version bump.
    _BOX64_RC="${HOME}/.box64rc"
    if $IS_BOOKWORM; then
        log "bookworm: skipping .box64rc write (box64 not installed)"
    elif [[ ! -f "$_BOX64_RC" ]] || ! grep -Fq "ry-crostini:${SCRIPT_VERSION}" "$_BOX64_RC" 2>/dev/null; then
        sed "s/@@VERSION@@/${SCRIPT_VERSION}/" <<'RCEOF' | write_file "$_BOX64_RC"
# box64 config — ry-crostini:@@VERSION@@. Manual edits below are overwritten on upgrade. See ptitSeb/box64 USAGE.md.

[default]
# Suppress verbose output
BOX64_LOG=0
# CALL/RET optimization — safe speedup on SC7180P
BOX64_DYNAREC_CALLRET=1
# Purge stale dynarec blocks — reclaims RAM on 4 GB device
BOX64_DYNAREC_PURGE=1
# DynaRec disk cache — 0=off, 1=generate+use, 2=use existing. Requires box64 ≥v0.3.8. Default 1.
BOX64_DYNACACHE=1
# Forward gap 512 (upstream default 128) — bigger blocks; only 0/128/256/512/1024 are valid.
BOX64_DYNAREC_FORWARD=512
# Map x86 PAUSE→ARM YIELD — better spinlock behavior, lower power on battery
BOX64_DYNAREC_PAUSE=1
# LSE atomics — A76 native. Default 0: unaligned LOCK from x86 can SIGBUS with =1. Enable per-game.
BOX64_DYNAREC_ALIGNED_ATOMICS=0
# Faster SMC handling. =1 can cause unexpected crashes (upstream USAGE.md). Enable per-game only.
BOX64_DYNAREC_DIRTY=0
# Expose every CPU core (upstream default) — no core limit for emulated x86 programs
BOX64_MAXCPU=0
...
RCEOF
        ...
        log ".box64rc up-to-date — skipping"
    fi
    unset _BOX64_RC
...
#!/usr/bin/env bash
# run-x86 — x86_64 emulation wrapper. Prefers box64; falls back to qemu-user. ry-crostini:@@VERSION@@.
...
# Detect ELF arch via raw header bytes (od). magic[4]=class, [18:2]=machine (LE). x86_64: 02/3e00; i386: 01/0300.
_detect_arch() {
    local magic
...
#!/usr/bin/env bash
# gog-extract — GOG installers on ARM64 sans Wine. .exe via innoextract, .sh via makeself. ry-crostini:@@VERSION@@.
...
case "$installer" in
    *.exe|*.EXE)
        ...
        # innoextract — extract Inno Setup .exe installers (most GOG Windows games).
        ...
        ;;
    *.sh|*.SH)
        # makeself — extract GOG Linux .sh installers. --noexec extracts without running install.
        _ms_score=0
        ...
        mkdir -p -- "$outdir"
        # Validate installer — must be Inno Setup (.exe) or makeself (.sh); else exit 2 with descriptive error.
...
#!/usr/bin/env bash
# run-game — launch on A76 big cores with elevated priority. SC7180P: 6-7=A76, 0-5=A55. ry-crostini:@@VERSION@@.
...
_cmd=("$@")
# Big-core part IDs: SC7180P 0x804, A76 0xd0b, A77 0xd0d, A78 0xd41, X1 0xd44, A710 0xd47, X2 0xd48, X3 0xd4e, X4 0xd81.
_big_cores=""
if [[ -d /sys/devices/system/cpu/cpu0 ]]; then
    ...
    if [[ "${_nparts:-0}" -ge 2 ]] && grep -qE '^CPU part[[:space:]]*:[[:space:]]*0x(804|d0b|d0d|d41|d44|d47|d48|d4d|d4e|d80|d81)' /proc/cpuinfo 2>/dev/null; then
        ...
        # Only pin if ≥2 distinct CPU parts AND at least one big-core part present. Else skip affinity.
...
# nice -n -5 needs CAP_SYS_NICE; ionice shares it, and -t ignores ionice failures ('none' scheduler).
...
# Skip GL error checks — unsafe globally on virgl, safe per-game through this wrapper.
export MESA_NO_ERROR=1
# Per-game: MESA_GLTHREAD=false run-game <cmd>. Not safe globally (crashes Firefox on X11/EGL with virgl).
...
if should_run_step 11; then
    # Put ~/.local/bin on PATH so the generated wrappers are probeable by the checks below
    ...
    if [[ -e /dev/dri/renderD128 ]]; then
        ...
        ((_verify_pass++)) || true
        # Re-apply gpu.conf env when probing GL — this shell session may predate the file
        _gpu_env_file="${HOME}/.config/environment.d/gpu.conf"
        _gl_env_args=()
        if [[ -r "$_gpu_env_file" ]]; then
            while IFS= read -r _gl_line; do
                # Trim leading whitespace before blank/comment tests so indented comments don't leak into env argv.
                _gl_line="${_gl_line#"${_gl_line%%[![:space:]]*}"}"
                [[ -z "$_gl_line" || "$_gl_line" == \#* ]] && continue
                [[ "$_gl_line" == *=* ]] || continue
                # Validate key charset (POSIX env identifier) before handing to env(1).
                _gl_key="${_gl_line%%=*}"
                [[ "$_gl_key" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] || continue
        ...
        unset _gpu_env_file _gl_line _gl_key
        if command -v glxinfo &>/dev/null; then
            {
                read -r GL_VENDOR
                read -r GL_RENDERER
            ...
            # Render-node existence != GPU acceleration. Detect llvmpipe/softpipe/swrast fallback explicitly.
            case "$GL_RENDERER" in
                *virgl*|*Virgl*|*VirGL*)
        ...
        if command -v vulkaninfo &>/dev/null; then
            ...
            # vulkaninfo --summary uses deviceName/apiVersion, not GPU name. Single awk pass on here-string.
            VK_GPU="$(awk -F= '/deviceName/{gsub(/^[ \t]+|[ \t]+$/, "", $2); print $2; exit}' <<<"$_vk_out" || true)"
            VK_API="$(awk -F= '/apiVersion/{gsub(/^[ \t]+|[ \t]+$/, "", $2); print $2; exit}' <<<"$_vk_out" || true)"
            unset _vk_out
            if [[ -n "$VK_GPU" ]]; then
                # Software Vulkan (lavapipe) is not acceleration — warn rather than pass
                case "$VK_GPU" in
    ...
    # Sommelier — check sommelier@ (Wayland) and sommelier-x@ (X11 bridge); either one is valid
    if systemctl --user list-units --type=service --state=active --no-legend \
       'sommelier@*.service' 'sommelier-x@*.service' 2>/dev/null | grep -q .; then
    ...
    # The three lines below read from config files on disk, not running state — labels say 'configured', not 'live'.
    logprintf '  GTK theme:     %s (configured)\n' "$(awk -F= '/^gtk-theme-name/{print $2; exit}' "${HOME}/.config/gtk-3.0/settings.ini" 2>/dev/null || echo 'default')"
    _xft_file="$(awk '/^Xft\.dpi:/ {print $2; exit}' "${HOME}/.Xresources" 2>/dev/null || true)"
    [[ -z "$_xft_file" ]] && _xft_file='default'
    logprintf '  Xft DPI:       %s (configured)\n' "$_xft_file"
    # Cross-check: does the running X server actually have Xft.dpi loaded? step 7 gates xrdb on $DISPLAY.
    if [[ -n "${DISPLAY:-}" ]] && command -v xrdb &>/dev/null; then
    ...
    unset _xft_file
    logprintf '  Font:          %s (configured)\n' "$(awk -F= '/^gtk-font-name/{print $2; exit}' "${HOME}/.config/gtk-3.0/settings.ini" 2>/dev/null || echo 'default')"
    logprintf '\n'
    ...
    if command -v pactl &>/dev/null; then
        PA_STATUS="$(timeout 5 pactl info 2>/dev/null | awk -F: '/^Server Name:/{sub(/^[ \t]+/, "", $2); print $2; exit}' || true)"
        if [[ -n "$PA_STATUS" ]]; then
    ...
    # bookworm has no box64 — check the trixie-only tools in a separate batch
    if $IS_BOOKWORM; then
        _parallel_check_tools \
    ...
    $IS_BOOKWORM && check_file "/etc/apt/sources.list.d/bookworm-backports.list" "bookworm-backports repo"
    ...
    # Qt theme packages — dpkg-query exact-match on 'install ok installed' (rejects half-installed)
    if [[ "$(dpkg-query -W -f='${Status}' qt5-gtk-platformtheme 2>/dev/null)" == "install ok installed" ]]; then
    ...
    if [[ "$(dpkg-query -W -f='${Status}' adwaita-qt 2>/dev/null)" == "install ok installed" ]]; then
    ...
    if [[ "$(dpkg-query -W -f='${Status}' qt6-gtk-platformtheme 2>/dev/null)" == "install ok installed" ]]; then
    ...
    # WirePlumber version probe — JSON .conf is ignored by 0.4.x. Detect version via --version output.
    if command -v wireplumber &>/dev/null; then
        # WirePlumber version probe — timeout 5 s; awk matches libwireplumber|^wireplumber, extracts first X.Y.Z.
        _wp_ver="$(timeout 5 wireplumber --version 2>/dev/null \
            | awk 'tolower($0) ~ /libwireplumber/ || /^wireplumber/ { if (match($0, /[0-9]+\.[0-9]+\.[0-9]+/)) { print substr($0, RSTART, RLENGTH); exit } }' \
            || true)"
        if [[ -n "$_wp_ver" ]]; then
            _wp_major="${_wp_ver%%.*}"
            _wp_rest="${_wp_ver#*.}"
    ...
    # PipeWire audio chain verification — check the SERVICE not just the SOCKET to avoid masking failed daemons.
    ...
    # Re-validate earlyoom --prefer regex shape. Stricter than step 3: require at least one known gaming process name.
    if [[ -s /etc/default/earlyoom ]]; then
        if grep -Eq '^EARLYOOM_ARGS=.*--prefer \([^)]*(retroarch|wine|dosbox|dosbox-x|scummvm|box64)[^)]*\)' /etc/default/earlyoom 2>/dev/null; then
    ...
    # earlyoom liveness — observe only. Verification must not mutate state; an unexpected death is a finding.
    ...
    # apt-daily.timer kept enabled for security visibility. Exact 'enabled' match via is-enabled.
    ...
    # Carry the cumulative failure count into the exit-code decision
...
if should_run_step 12; then
    ...
    # Wrappers without #! are exec'd via caller's shell — dash/cron/systemd break on bash-isms.
    for _wf in "${HOME}/.local/bin/run-x86" "${HOME}/.local/bin/gog-extract" "${HOME}/.local/bin/run-game"; do
        _wf_name="${_wf##*/}"
        if [[ ! -f "$_wf" ]]; then
            continue   # absence already reported by check_file above
        fi
        _wf_head="$(head -c2 -- "$_wf" 2>/dev/null || true)"
        if [[ "$_wf_head" == "#!" ]]; then
            logprintf '  %b✓%b  %-44s valid shebang\n' "$GREEN" "$RESET" "$_wf_name"
            ((_verify_pass++)) || true
        else
            logprintf '  %b✗%b  %-44s missing shebang — non-bash callers will fail\n' "$RED" "$RESET" "$_wf_name"
            ((_verify_fail++)) || true
        fi
    done
    unset _wf _wf_name _wf_head
    logprintf '\n'
...
if should_run_step 13; then
    ...
    if [[ $((_verify_pass + _verify_fail + _verify_warn)) -eq 0 ]]; then
        ...
        # All checks passed — clear the checkpoint so the next run starts fresh
        _progress_draw 13
        rm -f -- "$STEP_FILE"
    ...
    # Live-reload environment.d vars and restart sommelier. Parse KEY=VALUE (not source) for metachar safety.
    if [[ -d "${HOME}/.config/environment.d" ]]; then
        ...
        for _envf in "${HOME}/.config/environment.d/"*.conf; do
            while IFS= read -r _eline || [[ -n "$_eline" ]]; do
                ...
                # Strip optional surrounding double OR single quotes. Reject lone opening quote as malformed.
                if [[ "$_ev" == \"*\" && ${#_ev} -ge 2 ]]; then
                    _ev="${_ev:1:${#_ev}-2}"
                elif [[ "$_ev" == \'*\' && ${#_ev} -ge 2 ]]; then
                    _ev="${_ev:1:${#_ev}-2}"
                elif [[ "$_ev" == \"* || "$_ev" == \'* || "$_ev" == *\" || "$_ev" == *\' ]]; then
                    continue
                fi
                # Expand ${HOME}/$HOME — the only variable reference used in environment.d files in this script.
                _ev="${_ev//\$\{HOME\}/$HOME}"
                _ev="${_ev//\$HOME/$HOME}"
                export "$_ek=$_ev"
                _import_keys+=("$_ek")
